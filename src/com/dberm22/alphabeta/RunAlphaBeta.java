...
public class RunAlphaBeta implements EvalFunction{
  ...
  private static int numXs = 0;
  private static int numOs = 0;
  ...
  public static void main(String args[])
  {
    ...
    JFrame mainFrame = new JFrame("abMatch Tester");
    ...
    RunAlphaBeta eval = new RunAlphaBeta();

	Minimin tester = new Minimin();
	HashMap<Integer, Integer> permMap = tester.minimin(u_elements, v_elements, eval);
...
public <U, V> double execute(U u_element, V v_element) 
{
	if(numXs>numOs)
	{
		Point2D u_elementPos = B.get((Integer)u_element);
		Point2D v_elementPos = A.get((Integer)v_element);
